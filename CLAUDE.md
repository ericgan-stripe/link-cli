...
- Logging in again will replace existing access with new access specific to the new login attempt. Before starting a new login flow, run `auth status` and inspect the currently granted `scope` and `authorization_details`. Unless the caller explicitly intends to downgrade access, any follow-up `auth login` request should be a superset of those existing permissions.
- Re-running `auth login` prompts to add back any scopes that would be removed, and compares `authorization_details` coarsely by `type` only. Any requested `source` access counts as covering existing `source` access, and explicit `--authorization-detail` entries only need to match an existing granted detail's `type` to count as covered. In JSON/agent mode the prompt text goes to `stderr` and requires `y`/`n` on `stdin`.

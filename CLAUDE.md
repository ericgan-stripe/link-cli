...
- Link identity is negotiated internally. A `401` PrivateToken challenge consumes a matching token from `~/.link/aat-pool.json`, automatically refilling ten tokens from Link when necessary. Claims are issued on demand with `~/.link/holder-key.jwk`. Because claims nonces are single-use, the paid request is built from a fresh identity challenge rather than reusing the presentation that reached `402`.
- Identity-aware payments require the merchant's Stripe payment challenge to select `Payment-Authorization`; `Authorization` remains reserved for the Link attestation. All merchant requests use manual redirect handling so credentials never follow a redirect.
- In agent mode, the initial call returns the approval URL and an `_next.command` that is another `mpp pay` invocation with `--spend-request-id`. Do not expose raw attestations or identity presentations, and do not require a separate `spend-request retrieve` command.
